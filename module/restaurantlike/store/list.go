...
const timeLayout = "2006-01-02T15:04:05.99999"

func (s *sqlStore) GetRestaurantList(ctx context.Context, ids []int) (map[int]int, error) {
	...
	if err := s.db.Table(restaurantlikemodel.Like{}.TableName()).
		...
		Where("restaurant_id in (?)", ids).Group("restaurant_id").Find(&listLike).Error; err != nil {
